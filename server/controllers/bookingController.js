...
const checkAvailability = asyncHandler(async (req, res) => {
  try {
    ...
    const venueDocument = await Venue.findOne({ name: venueName });
    if (!venueDocument) {
      return res.status(404).json({ message: "Venue not found" });
    }

    const venueId = venueDocument._id; // Venue's ObjectId
    ...
    // Get bookings for the date and venue
    const existingBookings = await Booking.find({
      year: date.year,
      month: date.month,
      day: date.day,
      venue: venueId,  // Use the venue's ObjectId
...
const newBooking = asyncHandler(async (req, res) => {
  if (req.body.slots.length < 1) {
    res.status(400).json({
      message: "Please select slots and retry.",
    });
    return;
  }
  
  try {
    // Ensure consecutive time slots
    ...
    const venueName = req.body.venue;  // Assuming the venue name is passed in the request body

    // Find the venue by name to get the ObjectId
    ...
    // Fetch existing bookings for that date and venue
    const existingBookings = await Booking.find({
      year: date.year,
      month: date.month,
      day: date.day,
      venue: venue._id,  // Use the ObjectId of the venue
      status: { $not: { $eq: "cancelled" } },
    }).select({ slots: 1 });
    ...
    // Calculate total cost and check for conflicting bookings
    let total = 0;
    for (let i = 0; i < req.body.slots.length; i++) {
      if (bookedSlots.indexOf(req.body.slots[i].value) !== -1) {
        res.status(400).json({
          message: `One or more of the time slots you are trying to book have already been booked. Please retry with different times.`,
        });
        return;
    ...
    const userBalanceUpdate = await user.updateOne({ balance: newBalance });
    ...
    const newBooking = await Booking.create({
      ...
      status: "confirmed",
      date: unixDate,
      venue: venue._id,  // Save venue's ObjectId
    ...
    await Log.create({
      ...
      type: "booking",
      text: `${req.user.address} booked ${req.body.slots.length / 2} hour/s at ${venue.name}, on ${date.day}/${date.month}/${date.year}, totalling ${total}. User's balance is now: ${newBalance}.`,
    });
    ...
    //   } hour/s) on ${date.day}/${date.month}/${date.year} at ${venue.name} is confirmed.
    ...
    //       You have been added as a participant for the booking on ${date.day}/${date.month}/${date.year} at ${venue.name}.
