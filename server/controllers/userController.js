...
const userRegister = asyncHandler(async (req, res) => {
  ...
  if (!name_first || !name_last || !email || !password || !confirm_password) {
  ...
  if (newUser) {
    await Log.create({
      ...
      user_email: email,
      text: `New user ${name_first} ${name_last} (${email}) . Balance: ${0}`,
      type: "registration",
    });
...
const delAccount = asyncHandler(async (req, res) => {
  ...
  if (futureBookings > 0) {
    res
      ...
      .json({
        message:
          "You have upcoming bookings. Please cancel them and clear balance with the office",
      });
    return;
  }
  ...
  if (user.balance > 0) {
    res
      .status(400)
      .json({
        message: `You still have a balance of ${user.balance}. Please clear it with the office.`,
      });
    return;
  }
...
const fetchLogs = asyncHandler(async (req, res) => {
  ...
  if (
    req.params.duration != "3" &&
    req.params.duration != "7" &&
    req.params.duration != "30"
  ) {
